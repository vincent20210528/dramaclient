        ...
        <template #bottom>
            <el-card class="drama-main-card" shadow="never">
                ...
                <el-form :model="searchForm" inline class="search-form search-form--single-row">
                    ...
                    <el-form-item>
                        <el-select
                            v-model="searchForm.indexRange"
                            class="filter-select filter-select--index-range"
                            placeholder="剧集数范围"
                            clearable
                            filterable
                            allow-create
                        ...
                        >
                            <el-option
                                v-for="opt in indexRangeOptions"
                                :key="opt.value"
                                :label="opt.label"
                                :value="opt.value"
                            />
...
function normalizeIndexRangeInput(raw: string | undefined): string | undefined {
    const text = (raw ?? '').trim()
    if (!text) return undefined
    if (/^\d+$/.test(text)) return text
    const rangeMatched = text.match(/^(\d+)\s*-\s*(\d+)$/)
    if (rangeMatched) return `${rangeMatched[1]}-${rangeMatched[2]}`
    return undefined
}
...
async function loadListNow() {
    ...
    try {
        const normalizedIndexRange = normalizeIndexRangeInput(searchForm.indexRange)
...
function handleSearch() {
    const normalizedIndexRange = normalizeIndexRangeInput(searchForm.indexRange)
    if ((searchForm.indexRange ?? '').trim() && !normalizedIndexRange) {
        ElMessage.warning('剧集数范围仅支持纯数字或数字-数字格式')
        return
    }
    searchForm.indexRange = normalizedIndexRange ?? ''
    currentPage.value = 1
    syncRouteQuery()
...
function buildListQuery() {
    ...
    const normalizedIndexRange = normalizeIndexRangeInput(searchForm.indexRange)
    if (normalizedIndexRange) query.indexRange = normalizedIndexRange
...
onMounted(async () => {
    ...
    const page = Number(parseQueryString(route.query.page)) // note: 从 URL 里取页码
    if (Number.isFinite(page) && page > 0) currentPage.value = page //note: 处理页码 page 如果是合法正数，就把当前页码设为这个值
    const size = Number(parseQueryString(route.query.size)) // note: 从 URL 里取每页条数
    if ([10, 20, 50].includes(size)) pageSize.value = size // note: 只允许 10/20/50 三种合法值
    ...
    searchForm.indexRange = parseQueryString(route.query.indexRange) // note: 给搜索表单赋值
...
/* 换行排布、不出现横向滚动条；控件宽度收窄以适配常见内容区宽度 */
.search-form {
    display: flex;
    flex-wrap: wrap;
    align-items: center;
